...
namespace ArgoPantry.WPF.Behaviors;
...
public class ResponsivenessBehavior {
    ...
    private static void OnIsResponsiveChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
        if(d is FrameworkElement element) {
            FluentWindow window = (FluentWindow)Application.Current.MainWindow;
    ...
    private static void UpdateElement(FluentWindow window, FrameworkElement element) {
    ...
    private static Style CreateResponsivenessStyle(FrameworkElement element) {
        Style responsivenessStyle = new(element.GetType(), element.Style);

        foreach(Setter setter in GetHorizontalBreakpointSetters(element).Cast<Setter>()) {
